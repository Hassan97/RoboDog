...
static int handleSerialCommand(unsigned long ts, int key) {
    ...
    switch (key) {
        ...
        case ',':
            _cmdHandler.toggleButtons(BTN_LEFT);
            break;

        case '.':
            _cmdHandler.toggleButtons(BTN_RIGHT);
            break;
...
void setup() {
  ...
  ledcSetup(pwmChannel, freqq, resolutionn);

  ledcAttachPin(ENA_m1, pwmChannel);
  ledcAttachPin(ENB_m1, pwmChannel);
  ledcAttachPin(ENA_m2, pwmChannel);
  ledcAttachPin(ENB_m2, pwmChannel);
    Serial.begin(115200);
