...
void CommandHandler::setSwitches(int switches) {
    ...
    if ( isSet(shift, toggled, BTN_OFFSET_DEC2)) {
        Vector *pOffset = _quadruped.getTipOffset();
        pOffset->x -= 5;
        LOG("tip offset : %6.1f\n", pOffset->x);
    }

    if ( isSet(shift, toggled, BTN_OFFSET_INC2)) {
        Vector *pOffset = _quadruped.getTipOffset();
        pOffset->x += 5;
        LOG("tip offset : %5.1f\n", pOffset->x);
    }
if (isSet(shift, toggled, BTN_HEIGHT_DEC)) {
   ...
   if (isSet(shift, toggled, BTN_RIGHT)) {
        if (_vecMove.z >= (BODY_MIN_Z + 5)) {
            _vecMove.z -= 5;
            _vecMove.dump("DIR");
        }
  ...
  digitalWrite(IN_11, LOW);   //front left
  digitalWrite(IN_12, HIGH); //front left
  digitalWrite(IN_13, HIGH);//front right
  digitalWrite(IN_14, LOW); //front right
  digitalWrite(IN_21, LOW);  //back left
  digitalWrite(IN_22, HIGH); //back left
  digitalWrite(IN_23, HIGH); //back right
  digitalWrite(IN_24, LOW);  //back right
  delay(500);
       
  digitalWrite(IN_11, LOW);   //front left
  digitalWrite(IN_12, LOW); //front left
  digitalWrite(IN_13, LOW);//front right
  digitalWrite(IN_14, LOW); //front right
  digitalWrite(IN_21, LOW);  //back left
  digitalWrite(IN_22, LOW); //back left
  digitalWrite(IN_23, LOW); //back right
  digitalWrite(IN_24, LOW);  //back right
    }
    }
       

   if (isSet(shift, toggled, BTN_LEFT)) {
        if (_vecMove.z >= (BODY_MIN_Z + 5)) {
            _vecMove.z -= 5;
            _vecMove.dump("DIR");
        }
  ...
  digitalWrite(IN_21, HIGH);  //back left
  digitalWrite(IN_22, LOW); //back left
  digitalWrite(IN_23, LOW); //back right
  digitalWrite(IN_24, HIGH);  //back right
  delay(500);
  ...
  digitalWrite(IN_24, LOW);  //back right
    ...
    if (isSet(shift, toggled, BTN_FORWARD)) {
        if (_vecMove.z >= (BODY_MIN_Z + 5)) {
            _vecMove.z -= 5;
            _vecMove.dump("DIR");
        }
  ...
  digitalWrite(IN_24, LOW);  //back right
    ...
    if (isSet(shift, toggled, BTN_BACKWARD)) {
        if (_vecMove.z >= (BODY_MIN_Z + 5)) {
            _vecMove.z -= 5;
            _vecMove.dump("DIR");
        }
